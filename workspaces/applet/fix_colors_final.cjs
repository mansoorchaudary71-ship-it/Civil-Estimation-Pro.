...
files.forEach(file => {
    ...
    content = content.replace(/(['"`])([^'"`]*?)\1/g, (match, quote, inner) => {
        if ((inner.includes('bg-[#A855F7]') || inner.includes('bg-indigo-600') || inner.includes('bg-indigo-500') || inner.includes('bg-indigo-700') || 
             inner.includes('bg-purple-600') || inner.includes('bg-purple-500') || inner.includes('bg-purple-700') ||
             inner.includes('bg-pink-600') || inner.includes('bg-blue-600') || inner.includes('bg-slate-900')) 
